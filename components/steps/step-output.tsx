...
import { ArrowLeft, Download, Save, FileText, CheckCircle, AlertTriangle } from "lucide-react"
...
export function StepOutput({
  ...
  const generateOutput = async () => {
    ...
    try {
      ...
      // Simulate processing the source files with the mappings
      ...
      for (let i = 0; i < sourceFiles.length; i++) {
        ...
        setGenerationProgress(40 + (i + 1) * 20)

        // Apply mappings (simplified simulation)
        const transformedData = await applyMappings(content, activeMappings, file.name)
      ...
      const outputData = await formatOutput(processedData, outputFormat)
      setGenerationProgress(100)

      setOutputResult({
        data: outputData,
        format: outputFormat,
        fileName: `transformed-data-${Date.now()}${outputFormats.find((f) => f.value === outputFormat)?.extension}`,
        recordCount: processedData.reduce((sum, data) => sum + (data.records?.length || 0), 0),
  ...
  const formatOutput = async (processedData: any[], format: string) => {
    ...
    switch (format) {
      case "csv":
        ...
        const csvContent = [
          headers.join(","),
          ...allRecords.map((record) => headers.map((header) => `"${record[header] || ""}"`).join(",")),
        ].join("\n")
        return csvContent
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div>
              <p className="text-sm text-gray-600">Source Files</p>
              <p className="text-lg font-bold text-[#1a365d]">{sourceFiles.length}</p>
            </div>
            <div>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-gray-600">Save your final mapping configuration for future use</p>
            </div>
          ...
          <CardContent className="p-6">
            <div className="space-y-4">
              <div className="flex items-center justify-center space-x-2">
                ...
                <span className="text-[#1a365d] font-medium">Generating output file...</span>
              </div>
              <Progress value={generationProgress} className="w-full" />
              <p className="text-sm text-gray-600 text-center">
                {generationProgress < 40 && "Reading source files..."}
                {generationProgress >= 40 && generationProgress < 80 && "Applying field mappings..."}
                {generationProgress >= 80 && "Formatting output data..."}
              </p>
            </div>
          </CardContent>
        </Card>
      ...
      {outputResult && (
        <Card className="border-blue-200 bg-blue-50">
          <CardContent className="p-4">
            <div className="text-center">
              <h4 className="font-medium text-blue-800 mb-2">🎉 Transformation Complete!</h4>
              <p className="text-sm text-blue-700">
                You have successfully completed the 4-step workflow. Your data has been transformed and is ready for
                use. You can start a new transformation or download your mapping configuration for future use.
              </p>
            </div>
          </CardContent>
        </Card>
